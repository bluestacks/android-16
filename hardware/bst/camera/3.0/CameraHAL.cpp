...
void CameraHAL::enumSensorSet()
{
    ...
    memcpy(mSets[BACK_CAMERA_ID].mPropertyName, "bstcamera", strlen("bstcamera"));
    ...
    memcpy(mSets[FRONT_CAMERA_ID].mPropertyName, "bstcamera", strlen("bstcamera"));
...
int32_t CameraHAL::matchDevNodes()
{
    ...
    for (int i = 0; i < 2; i++) {
        ...
        memcpy(node->devNode, "/dev/video0", strlen("/dev/video0"));
