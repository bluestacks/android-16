...
int32_t VideoStream::configure(sp<Stream> stream)
{
    ...
    params->mId = stream->id();

    ALOGI("%s: w:%d, h:%d, sensor format:0x%x, stream format:0x%x, fps:%d, num:%d id:%d",
           __func__, params->mWidth, params->mHeight, params->mFormat, stream->format(), params->mFps, params->mBuffers, params->mId);
...
int32_t VideoStream::handleConfigureLocked(ConfigureParam* params)
{
    ...
    if ((mWidth == (uint32_t)params->mWidth) && (mHeight == (uint32_t)params->mHeight)
         && (mFormat == (int32_t)params->mFormat)  && (params->mIsJpeg || (mFps == (uint32_t)params->mFps))) {
        ALOGI("%s, same config, res %dx%d, fmt 0x%x, fps %d, %d, jpg %d id %d",
          __func__, mWidth, mHeight, mFormat, mFps, params->mFps, params->mIsJpeg, params->mId);
        flushDevLocked();
        return 0;
    }
    ...
    ret = onDeviceConfigureLocked(params->mId);
    if (ret != 0) {
