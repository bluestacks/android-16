...
BstCamera::BstCamera(int32_t id,
                         int32_t facing,
                         int32_t orientation,
                         char* path)
...
PixelFormat BstCamera::getPreviewPixelFormat()
{
    return HAL_PIXEL_FORMAT_RGBA_8888;
}

int BstCamera::getCaptureMode(int width, int height)
{
    int capturemode = -1;
...
status_t BstCamera::initSensorStaticData()
{
    ...
    int maxWidth = fsize.stepwise.max_width;
    ...
    mPreviewResolutions[previewCnt++] = 320;
    mPreviewResolutions[previewCnt++] = 240;

    mPictureResolutions[pictureCnt++] = 640;
    mPictureResolutions[pictureCnt++] = 480;
    mPictureResolutions[pictureCnt++] = 320;
    mPictureResolutions[pictureCnt++] = 240;
    ...
    mMaxWidth = mPictureResolutions[0];
    mMaxHeight = mPreviewResolutions[1];
    ...
    ALOGI(" mFocalLength:%f, mPhysicalWidth:%f, mPhysicalHeight %f, mMaxJpegSize %d",
                mFocalLength,
                mPhysicalWidth,
                mPhysicalHeight,
                mMaxJpegSize);
...
// configure device.
int32_t BstCamera::OvStream::onDeviceConfigureLocked(int id)
{
    if (mDev <= 0) {
    ...
    return MMAPStream::onDeviceConfigureLocked(id);
}
