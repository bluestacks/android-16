...
int V4L2Camera::Open (const char *device)
{
    ...
    ALOGD_IF(DEBUG, "Open %s OK", device);
...
{
    ALOGD_IF(DEBUG, "V4L2Camera::Init(%d, %d, %d)", width, height, fps);
    ...
    for (i = 0; i < m_AllFmts.size(); i++) {
        ...
        if (sd.getWidth() >= width &&
            ...
            ALOGD_IF(DEBUG, "Trying format: (%d x %d), Fps: %d [difArea:%d, difFps:%d, cDifArea:%d, cDifFps:%d]",sd.getWidth(),sd.getHeight(),sd.getFps(), difArea, difFps, closestDArea, closestDFps);
            if (closestDArea < 0 ||
    ...
    ALOGD_IF(DEBUG, "Selected format: (%d x %d), Fps: %d",closest.getWidth(),closest.getHeight(),closest.getFps());
    ...
    if (videoIn->format.fmt.pix.pixelformat == V4L2_PIX_FMT_JPEG ||
        ...
        if(ioctl(fd,VIDIOC_G_JPEGCOMP, &videoIn->jpegcomp) >= 0) {
            ALOGD_IF(DEBUG, "VIDIOC_G_COMP:\n");
            ALOGD_IF(DEBUG, "    quality:      %i\n", videoIn->jpegcomp.quality);
            ALOGD_IF(DEBUG, "    APPn:         %i\n", videoIn->jpegcomp.APPn);
            ALOGD_IF(DEBUG, "    APP_len:      %i\n", videoIn->jpegcomp.APP_len);
            ALOGD_IF(DEBUG, "    APP_data:     %s\n", videoIn->jpegcomp.APP_data);
            ALOGD_IF(DEBUG, "    COM_len:      %i\n", videoIn->jpegcomp.COM_len);
            ALOGD_IF(DEBUG, "    COM_data:     %s\n", videoIn->jpegcomp.COM_data);
            ALOGD_IF(DEBUG, "    jpeg_markers: 0x%x\n", videoIn->jpegcomp.jpeg_markers);
        } else {
            ALOGE("VIDIOC_G_COMP:");
            if(errno == EINVAL) {
                videoIn->jpegcomp.quality = -1; //not supported
...
{
    ALOGD_IF(DEBUG, "V4L2Camera::EnumFrameIntervals: pixfmt: 0x%08x, w:%d, h:%d",pixfmt,width,height);
    ...
    ALOGD_IF(DEBUG, "\tTime interval between frame: ");
    while (ioctl(fd,VIDIOC_ENUM_FRAMEINTERVALS, &fival) >= 0)
    {
        fival.index++;
        if (fival.type == V4L2_FRMIVAL_TYPE_DISCRETE) {
            ALOGD_IF(DEBUG, "%u/%u", fival.discrete.numerator, fival.discrete.denominator);

            m_AllFmts.add( SurfaceDesc( width, height, fival.discrete.denominator ) );
            list_fps++;
        } else if (fival.type == V4L2_FRMIVAL_TYPE_CONTINUOUS) {
            ALOGD_IF(DEBUG, "{min { %u/%u } .. max { %u/%u } }",
...
{
    ALOGD_IF(DEBUG, "V4L2Camera::EnumFrameSizes: pixfmt: 0x%08x",pixfmt);
    int fsizeind = 0;
    ...
    while (ioctl(fd, VIDIOC_ENUM_FRAMESIZES, &fsize) >= 0) {
        ...
        if (fsize.type == V4L2_FRMSIZE_TYPE_DISCRETE) {
            ALOGD_IF(DEBUG, "{ discrete: width = %u, height = %u }",
                fsize.discrete.width, fsize.discrete.height);

            fsizeind++;

            if (!EnumFrameIntervals(pixfmt,fsize.discrete.width, fsize.discrete.height))
                ALOGD_IF(DEBUG, "  Unable to enumerate frame intervals");
        } else if (fsize.type == V4L2_FRMSIZE_TYPE_CONTINUOUS) {
            ALOGD_IF(DEBUG, "{ continuous: min { width = %u, height = %u } .. "
                "max { width = %u, height = %u } }",
                fsize.stepwise.min_width, fsize.stepwise.min_height,
                fsize.stepwise.max_width, fsize.stepwise.max_height);
            ALOGD_IF(DEBUG, "  will not enumerate frame intervals.\n");
        } else if (fsize.type == V4L2_FRMSIZE_TYPE_STEPWISE) {
            ALOGD_IF(DEBUG, "{ stepwise: min { width = %u, height = %u } .. "
                "max { width = %u, height = %u } / "
    ...
    if (fsizeind == 0) {
        ...
        for (i = 0 ; i < (sizeof(defMode) / sizeof(defMode[0])); i++) {
            ...
            if (ioctl(fd,VIDIOC_TRY_FMT, &fmt) >= 0) {
                ALOGD_IF(DEBUG, "{ ?GSPCA? : width = %u, height = %u }\n", fmt.fmt.pix.width, fmt.fmt.pix.height);
...
{
    ALOGD_IF(DEBUG, "V4L2Camera::EnumFrameFormats");
    ...
    while (ioctl(fd,VIDIOC_ENUM_FMT, &fmt) >= 0) {
        fmt.index++;
        ALOGD_IF(DEBUG, "{ pixelformat = '%c%c%c%c', description = '%s' }",
...
{
    ALOGD_IF(DEBUG, "V4L2Camera::getAvailableSizes");
...
{
    ALOGD_IF(DEBUG, "V4L2Camera::getAvailableFps");
