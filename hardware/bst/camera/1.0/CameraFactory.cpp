...
{
    ALOGD_IF(DEBUG, "CameraFactory::CameraFactory");
    mCamera = NULL;
    mCameraDevices = NULL;
    mCameraFacing = NULL;
...
{
    ALOGD_IF(DEBUG, "CameraFactory::cameraDeviceOpen: id = %d", camera_id);
    ...
    *device = NULL;
    ...
    if (!mCamera[camera_id]) {
        mCamera[camera_id] = new CameraHardware(module, mCameraDevices[camera_id], mCameraFacing[camera_id]);
    }
...
{
    ALOGD_IF(DEBUG, "CameraFactory::getCameraNum: %d", mCameraNum);
    return mCameraNum;
}

int CameraFactory::getCameraInfo(int camera_id, struct camera_info* info)
{
    ALOGD_IF(DEBUG, "CameraFactory::getCameraInfo: id = %d, info = %p", camera_id, info);
...
{
    ALOGD_IF(DEBUG, "CameraFactory::parseConfig: configFile = %s", configFile);
    ...
    if (config != NULL) {
        ...
        while (fgets(line, sizeof line, config) != NULL) {
            ...
            if (strcmp(arg1, "front") == 0) {
                ...
                ALOGD_IF(DEBUG, "CameraFactory::parseConfig: Unrecognized config line '%s'", line);
            }
        }
    } else {
        ALOGD_IF(DEBUG, "%s not found, using camera configuration defaults", CONFIG_FILE);

        if (access(DEFAULT_DEVICE_BACK, F_OK) != -1){
            ALOGD_IF(DEBUG, "Found device %s", DEFAULT_DEVICE_BACK);
            newCameraConfig(CAMERA_FACING_BACK, DEFAULT_DEVICE_BACK, 0);
        }
        if (access(DEFAULT_DEVICE_FRONT, F_OK) != -1){
            ALOGD_IF(DEBUG, "Found device %s", DEFAULT_DEVICE_FRONT);
...
void CameraFactory::newCameraConfig(int facing, const char* location, int orientation)
{
    ...
    ALOGD_IF(DEBUG, "CameraFactory::newCameraConfig: %d -> %s (%d)",
...
int CameraFactory::device_open(const hw_module_t* module,
                                       ...
                                       hw_device_t** device)
{
    ALOGD_IF(DEBUG, "CameraFactory::device_open: name = %s", name);
...
{
    ALOGD_IF(DEBUG, "CameraFactory::get_number_of_cameras");
...
{
    ALOGD_IF(DEBUG, "CameraFactory::get_camera_info");
